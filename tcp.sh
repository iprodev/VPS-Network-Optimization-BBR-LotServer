...
#	Description: BBR+BBR magic revision+BBRplus+Lotserver
#	Version: 1.0.0
#	Author: iProDev
#	Blog: https://iprodev.com/
#   It is recommended to use the bbr directly opened by the kernel above 5.5 for the best speed
#=================================================

sh_ver="1.0.0"
github="raw.githubusercontent.com/chiakge/Linux-NetSpeed/master"
iprodev_github="raw.githubusercontent.com/iprodev/VPS-Network-Optimization-BBR-LotServer/master"
...
installlot(){
	...
	stty erase '^H' && read -p "The Lotserver needs to be restarted after the VPS is restarted. Do you want to restart it now? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS restarting..."
		reboot
...
# Compile and enable BBR magic modification
startbbrmod(){
	remove_all
	if [[ "${release}" == "centos" ]]; then
...
remove_all(){
  ...
  sed -i '/fs.file-max/d' /etc/sysctl.conf
	...
	clear
	echo -e "${Info}: Clearing all accelerations is successful."
	sleep 1s
}
...
#Update Script
Update_Shell(){
	echo -e "The current version is [ ${sh_ver} ], to start checking for the latest version..."
	sh_new_ver=$(wget --no-check-certificate -qO- "http://${iprodev_github}/tcp.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} Failed to check latest version!" && start_menu
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		echo -e "New version found[ ${sh_new_ver} ], do you want to update?[Y/n]"
		read -p "(Default: y):" yn
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -N --no-check-certificate http://${iprodev_github}/tcp.sh && chmod +x tcp.sh
...
———————————— Acceleration Management ————————————
 ...
 ${Green_font_prefix}5.${Font_color_suffix} Use the BBR magic modification to speed up
 ${Green_font_prefix}6.${Font_color_suffix} Use the violent BBR magic revision to accelerate (does not support some systems)
 ${Green_font_prefix}7.${Font_color_suffix} Accelerate with BBRplus
 ${Green_font_prefix}8.${Font_color_suffix} Accelerate with Lotserver
...
————————————————————————————————" && echo
	...
	if [[ ${kernel_status} == "noinstall" ]]; then
		...
		echo -e " Current Status: ${Green_font_prefix}Installed${Font_color_suffix} ${_font_prefix}${kernel_status}${Font_color_suffix} accelerated core, ${Green_font_prefix}${run_status}${Font_color_suffix}"
		
	fi
echo
read -p " Please enter key in numbers [0-11]:" num
case "$num" in
	0)
	Update_Shell
	;;
...
detele_kernel(){
	if [[ "${release}" == "centos" ]]; then
		...
		if [ "${rpm_total}" > "1" ]; then
			echo -e "Detected ${rpm_total} The rest of the kernel, start uninstalling......"
			...
			echo -e " Detected incorrect number of cores, please check!" && exit 1
		fi
	elif [[ "${release}" == "debian" || "${release}" == "ubuntu" ]]; then
		deb_total=`dpkg -l | grep linux-image | awk '{print $2}' | grep -v "${kernel_version}" | wc -l`
		if [ "${deb_total}" > "1" ]; then
			echo -e "Detected ${deb_total} The rest of the kernel, start uninstalling......"
	...
	if [[ ${kernel_status} == "Lotserver" ]]; then
		...
		if [[ ${run_status} == "bbr" ]]; then
			...
			if [[ ${run_status} == "tcp_nanqinlang" ]]; then
				run_status="Violent BBR magic revision started successfully"
			else 
